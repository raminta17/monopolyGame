...
const Dice = ({ error, setError, taxBoxes, chanceSquares, gameBoard }) => {
	const dice = [
		{
			dice: 1,
	...
	function rollDice() {
		...
		gameBoard.map((gameDiv, index) => {
			if (position === index && 'color' in gameDiv)
				setError(`You are on ${gameDiv.name}`)
		})
		taxBoxes.map(taxBox => {
		...
		chanceSquares.map(chanceSquare => {
			if (position === Number(chanceSquare.id)) {
				let randomChance =
					Math.floor(Math.random() * chanceSquare.chance * 2 + 1) -
					chanceSquare.chance
				if (randomChance > 0) {
	...
	return (
		<div className='diceCont'>
			<div>
				{rolledValue && (
					<img className='diceImg' src={dice[rolledValue - 1].image} />
				)}
			</div>
